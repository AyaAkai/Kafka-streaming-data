 ...
 */
function conditional(strings, ...expr) {
    return expr.reduce((accumulator, currentExpr, index) => {
        if (!!currentExpr) {
            return accumulator + strings[index] + currentExpr;
        }
        return accumulator;
    }, '') + strings.slice(-1); // A bug in the Inlay Hints feature is fixed by this redundant `as string` casting
}
