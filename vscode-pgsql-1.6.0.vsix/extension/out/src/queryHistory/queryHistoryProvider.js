...
class QueryHistoryProvider {
    ...
    refresh(ownerUri, timeStamp, hasError) {
        ...
        const tooltip = this.createHistoryNodeTooltip(queryString, connectionLabel, timeStampString);
    ...
    addCopilotQuery(historyLabel, queryString, ownerUri, hasError) {
        const timeStamp = new Date();
        const timeStampString = timeStamp.toLocaleString();
        const connectionLabel = this.getConnectionLabel(ownerUri);
        const historyNodeLabel = historyLabel;
        const tooltip = this.createHistoryNodeTooltip(queryString, connectionLabel, timeStampString);
        const node = new queryHistoryNode_1.QueryHistoryNode(historyNodeLabel, tooltip, queryString, ownerUri, timeStamp, connectionLabel, !hasError);
        this.addNode(node);
    }
    ...
    /**
     * Copy a query history listing
     */
    copyQueryHistoryEntry(node) {
        return __awaiter(this, void 0, void 0, function* () {
            const queryString = node.queryString;
            try {
                yield vscode.env.clipboard.writeText(queryString);
                this._vscodeWrapper.showInformationMessage("Query text copied to clipboard.");
            }
            catch (err) {
                this._vscodeWrapper.showErrorMessage("Failed to copy query history entry: " + err.message);
            }
        });
    }
    /**
     ...
     */
    createHistoryNodeLabel(queryString, connectionLabel) {
        const queryStringTrimmed = Utils.limitStringSize(queryString).trim();
        const connectionLabelTrimmed = Utils.limitStringSize(connectionLabel).trim();
        return `${queryStringTrimmed} : ${connectionLabelTrimmed}`;
    }
    /**
     ...
     */
    createHistoryNodeTooltip(queryString, connectionLabel, timeStamp) {
