...
class FormWebviewController extends reactWebviewPanelController_1.ReactWebviewPanelController {
    ...
    handleFormAction(state, payload) {
        return __awaiter(this, void 0, void 0, function* () {
            if (payload.event.isAction) {
                ...
                yield this.validateForm(this.state.formState, payload.event.propertyName);
     ...
     */
    validateForm(formTarget, propertyName) {
        return __awaiter(this, void 0, void 0, function* () {
            const erroredInputs = [];
            if (propertyName) {
                const component = this.state.formComponents[propertyName];
                if (component && component.validate) {
                    component.validation = component.validate(this.state, formTarget[propertyName]);
                ...
                this.getActiveFormComponents(this.state)
                    ...
                    if (c.hidden) {
                        ...
                        if (c.validate) {
                            c.validation = c.validate(this.state, formTarget[c.propertyName]);
