    ...
    openSavedFile(filePath, format) {
        ...
        if (format === "excel") {
            ...
            opener(filePath, undefined, (error, stdout, stderr) => {
                if (error) {
                    if (error.code === "ENOENT" && error.path === "xdg-open") {
                        // This typically means that xdg-utils haven't been installed on a Linux system
                        // and couldn't open an Excel file. Show a more user-friendly error message.
                        self._vscodeWrapper.showErrorMessage(LocalizedConstants.msgOpenFileFailedNoXdg);
                    }
                    else {
                        self._vscodeWrapper.showErrorMessage(error);
                    }
                    self._vscodeWrapper.logToOutputChannel([error.message, stdout, stderr]);
                }
            });
        }
        else {
